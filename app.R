...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      p("データはNPB公式から. 2012年以後の1, 2軍での個人成績比較. データは17/9/15時点で更新されていたもの. 曲線は局所的な移動平均."),
      ...
      p("下のスライドバーで解析に含めるサンプルサイズの閾値を変更して遊ぼう! BattingではPA, PitchingではTBFを使用. "),
      sliderInput("range",
                  "1軍での最低PA/TBF:",
                  min = 1,
                  max =2000,
      ...
      helpText("特定の選手のみを見たい場合、名前を入力. 要完全一致 (スペースは全角で). 正しい名前を入力して表示されない時は上のスライドバーを左にすると見えるようになるかも."),
      ...
      helpText("dataの性格上サンプリングバイアスのかたまりなので注意 (一軍で運が悪い → 2軍でのサンプル数が多い、など)."),
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("wOBA/FIP", plotOutput("plot8")),
                  tabPanel("AVG/ERA", plotOutput("plot9")),
                  tabPanel("HR", plotOutput("plot10")),
                  tabPanel("K", plotOutput("plot11")),
                  tabPanel("BB", plotOutput("plot12")),
                  tabPanel("K-BB", plotOutput("plot13")),
                  tabPanel("BABIP", plotOutput("plot14")),
                  tabPanel("wOBA/FIP_L", plotOutput("plot1")),
                  tabPanel("AVG/ERA_L", plotOutput("plot7")),
                  tabPanel("HR_L", plotOutput("plot2")),
                  tabPanel("K_L", plotOutput("plot3")),
                  tabPanel("BB_L", plotOutput("plot4")),
                  tabPanel("K-BB_L", plotOutput("plot5")),
                  tabPanel("BABIP_L", plotOutput("plot6")),
